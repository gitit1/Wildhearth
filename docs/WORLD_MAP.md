...
| Town | ✅ | **Built and walkable** (V2-B1, verified 2026-07-14): the coastal town strip (`zones.ts` TOWN_*) — an inn (exterior, `buildings/inn`), a seafront + walkable town dock, a town square with its own busking spot (`TOWN_BUSK_SPOT`, added V2-B1) beside the player's relocated stall, 4 specialised merchant stalls (general/fishmonger/greengrocer/tailor, distinct spare-stall sprites), 5 distinct homes, and a stable. Depth-sorted, collision-clean. Inn INTERIOR (enterable + paid sleep) is still ⬜ — scheduled V2-B12. Distinct from the market square above (v1's commerce hub) |
