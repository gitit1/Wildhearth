...
|---|---|---|
| The Farm (rundown state) | ✅ | VISION Opening Arc/Sequence, ROADMAP_EXPANSION Phase 0 item below |
| The Farm (renovated states) | 🟡 | VISION #8 Housing tiers — tiers named, exact visual milestones not walked through |
| House interior | ✅ | VISION #13 Needs + ROADMAP_EXPANSION Phase 0 item 5 — bare/broken tier-1 state fully detailed |
| Market/dock area | 🟡 | ROADMAP_EXPANSION Phase 0 item 3 — confirmed separate & walkable, layout not detailed |
| River (fishing) | ✅ | Fishing/junk mechanics + Riverside Fisherwoman (dialogue/shop/teaching/boats) + bird-watching + boat-gated fish/diving + sailing — ROADMAP_EXPANSION Phase 1 item 5 |
| Forest edge (foraging) | 🟡 | Foraging mechanic detailed, the place itself not walked |
...
| Town | 🟡 | Building list named (inn, 3 merchants, 5–8 homes) in ROADMAP_EXPANSION Phase 1, not walked through as a place; confirmed to be coastal — a seafront belongs near the town, not the farm |
| Coast (near town) | ⬜ | Named as its own region in VISION, no content yet — distinct from the river |
| Road (farm → town) | 🟡 | Confirmed to exist + gradual minimap reveal + ambient wildlife (rabbits/birds/deer) + at least one neighboring farm + Sims-style paid fast travel — ROADMAP_EXPANSION Phase 1 items 1–2 |
| First NPC roster | 🟡 | The Riverside Fisherwoman is the first confirmed NPC (ROADMAP_EXPANSION Phase 1 item 5); the 5–8 town NPCs are still ⬜, tracked in OPEN_QUESTIONS.md |
...
House interior — done. Suggested order for what's left, since it follows
how a player would actually move: the road → the market → the
river/forest edge → the town.
