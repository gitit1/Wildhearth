...
// W2a: the new generic stall's awning is a muted terracotta (hue ~0-18°,
// re-measured off the UO-mood sprite's own histogram — the old 334-6 band
// missed the 15° cloth pixels); satMin floor keeps the wooden counter/posts
// (low saturation) out of the recolour.
const STALL_AWNING_BAND: HueBand = { hueMin: 328, hueMax: 22, satMin: 0.32 };

/**
 * W2a ROOFLINE METADATA (the interiors constraint). For every straight-on
 * building sprite: the roof band's pixel y-range `[topOfSprite, eavesLine]` in
 * the sprite's OWN pixel space (measured alpha bbox top → the widest row in the
 * top ~55% = the eaves). The future Sims-style roof-hide (docs/DECISIONS.md
 * interiors entry) masks this band to reveal the interior when the player is
 * inside. Straight-on facades make the split a clean horizontal line; these are
...
//  top, positioned in the SPRITE's own pixel space (re-tuned for the W2a
//  UO-mood straight-on farmhouse/barn, read off a 16px coordinate grid: the
//  farmhouse's mossy green roof plane fills the upper-right, so the torn hole
//  sits on clear green mid-roof at ≈(112-152, 72-106); its lower-right window
//  ≈(119-146, 112-137). The barn's X-braced double doors centre ≈(104,132);
//  the right timber wall below the eave gives a missing-plank gap ≈(167,118)).
